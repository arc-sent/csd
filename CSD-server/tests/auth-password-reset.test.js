...
function verifyCode(app, email, code) {
  return request(app).post('/api/auth/password-reset/verify').send({ email, code });
}

function confirmReset(app, resetToken, newPassword) {
  return request(app).post('/api/auth/password-reset/confirm').send({ resetToken, newPassword });
}

describe('Auth module: восстановление пароля админа до входа', () => {
  let app;
  const emails = [
    'test-auth-reset@chesslab.local',
    'test-auth-reset-cooldown@chesslab.local',
    'test-auth-reset-lockout@chesslab.local',
    'test-auth-reset-success@chesslab.local'
  ];
  ...
  it('request всегда отвечает {sent: true}, даже для несуществующего email — иначе оракул для перебора', async () => {
    const res = await request(app).post('/api/auth/password-reset/request').send({ email: 'no-such-admin@chesslab.local' });
  ...
  it('код для несуществующего email отклоняется той же общей ошибкой', async () => {
    const res = await verifyCode(app, 'no-such-admin@chesslab.local', '123456');
    expect(res.status).toBe(400);
    expect(res.body.error).toBe('Неверный или истёкший код');
  });
  ...
  it('resetToken меняет пароль — вход старым паролем перестаёт работать, новым начинает', async () => {
    ...
    await prisma.adminUser.create({ data: { email, passwordHash: await bcrypt.hash(PASSWORD, 10) } });
    ...
    const newLogin = await request(app).post('/api/auth/login').send({ email, password: 'brand-new-password-123' });
  ...
  it('resend раньше 60 секунд после запроса не шлёт письмо повторно, но всё равно отвечает {sent: true}', async () => {
    ...
    const logSpy = jest.spyOn(console, 'log').mockImplementation(() => {});
    const res = await request(app).post('/api/auth/password-reset/request').send({ email: emails[1] });
