...
const MOTIFS = [
  ...
  },
  {
    ...
    icon: '♚',
    title: 'Связка',
    text: 'Угроза дальнобойной фигурой (слон, ладья, ферзь) королю (фигуре, пункту) соперника через любую его фигуру.'
  },
  {
    code: '2.9',
    icon: '♖',
...
export function ExamplePanel({example, title, label}) {
      ...
      <div className="grid sm:grid-cols-[.95fr_1.05fr]">
        <div className="flex items-center justify-center p-4 sm:p-6 bg-panel-deep">
          <BoardFrame flipped={flipped} coordClassName="text-faint" className="w-full max-w-[300px]">
            <Board large position={example.position} flipped={flipped} label={label || `Пример на мотив «${title}»`} />
          </BoardFrame>
        </div>
        <div className="flex flex-col justify-center p-5 sm:p-6">
          ...
          {example.note && <p className="text-[13px] text-ink leading-[1.6] mt-2.5">{example.note}</p>}
          <div className={example.note ? 'mt-4 pt-4 border-t border-panel-line' : 'mt-3'}>
